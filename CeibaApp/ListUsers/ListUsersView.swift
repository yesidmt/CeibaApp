...
extension ListUsersView: UITableViewDataSource,UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if filterUsers[0].data.count == 0 {
            ...
            guard let cell = tableView.dequeueReusableCell(withIdentifier: "SessionUserCell", for: indexPath) as? SessionUserCell else {
                        fatalError("Unable to dequeue Cell")}
            let it = Item(id: filterUsers[0].data[indexPath.row].id,name: filterUsers[0].data[indexPath.row].name,phone: filterUsers[0].data[indexPath.row].phone,email:filterUsers[0].data[indexPath.row].email)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        presenter?.goToPostUser(user:filterUsers[0].data[indexPath.row])
    }
}
